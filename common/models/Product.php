...
class Product extends \yii\db\ActiveRecord
{
    ...
    public static function inCart($customer_id, $product_id)
    {
        ...
        if ($order)
        {
            $cartItem = Cart::find()->where(['order_id' => $order->order_id, 'product_id' => $product_id])->orderBy(['cart_id' => SORT_DESC])->one();
        } else {
            $cartItem = null;
        }
        if (is_null($cartItem)){
    ...
    public static function getMeasure($measure)
    {
        $fullMeasure = '';
        switch ($measure)
        {
            case 'кг':
                $fullMeasure = 'килограмм';
                break;
            case 'гр':
                $fullMeasure = 'грамм';
                break;
